...
Klient bezpoœrednio po uruchomieniu tworzy kolejkê z unikalnym kluczem IPC  i wysy³a jej klucz komunikatem do serwera. Po otrzymaniu takiego komunikatu, serwer otwiera kolejkê klienta, przydziela klientowi identyfikator (np. numer w kolejnoœci zg³oszeñ) i odsy³a ten identyfikator do klienta (komunikacja w kierunku serwer->klient odbywa siê za pomoc¹ kolejki klienta). Po otrzymaniu identyfikatora, klient rozpoczyna wysy³anie zleceñ do serwera (w pêtli), zlecenia s¹ czytane ze standardowego wyjœcia w postaci typ_komunikatu albo z pliku tekstowego w którym w ka¿dej linii znajduje siê jeden komunikat.


Rodzaje zleceñ:
* Us³uga lustra (MIRROR):
Klient wysy³a ci¹g znaków. Serwer odsy³a ten sam ci¹g z powrotem w odwrotnej kolejnoœci. Klient po odebraniu wysy³a go na standardowe wyjœcie.
* Us³uga kalkulatora (CALC):
Klient wysy³a proste dzia³anie arytmetyczne (+ - \* /) Serwer oblicza i odsy³a wynik. Klient po odebraniu wysy³a go na standardowe wyjœcie. Dzia³ania mo¿na przesy³aæ w postaci np CALC 2+3, CALC 3\*4 albo ADD 1 2, MUL 3 4, SUB 5 4, DIV 4 3 - wariant do wyboru.
* Us³uga czasu (TIME):
...
* Nakaz zakoñczenia (END):
Po odebraniu takiego zlecenia serwer zakoñczy dzia³anie, jak tylko opró¿ni siê kolejka zleceñ (zostan¹ wykonane wszystkie pozosta³e zlecenia). Klient nie czeka na odpowiedŸ.

Poszczególne rodzaje komunikatów nale¿y identyfikowaæ za pomoc¹ typów komunikatów systemu V. Klucze dla kolejek maj¹ byæ generowane na podstawie œcie¿ki *$HOME*. Ma³e liczby do wygenerowania kluczy oraz rodzaje komunikatów maj¹ byæ zdefiniowane we wspólnym pliku nag³ówkowym. Dla uproszczenia mo¿na za³o¿yæ, ¿e d³ugoœæ komunikatu (lub maksymalna d³ugoœæ ³añcucha znaków przy us³udze lustra) jest ograniczona pewn¹ sta³¹ (jej definicja powinna znaleŸæ siê w pliku nag³ówkowym).
...
## Zadanie 2. POSIX
Zrealizuj zadanie analogiczne do Zadania 1, wykorzystuj¹c kolejki komunikatów POSIX.
